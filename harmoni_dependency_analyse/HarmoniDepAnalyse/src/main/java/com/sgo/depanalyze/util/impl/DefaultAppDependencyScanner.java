 ...
 */
public class DefaultAppDependencyScanner implements IDependencyScanner {
    //
    /** The logger. */
    private static Logger logger = Logger.getLogger(DefaultAppDependencyScanner.class);
    ...
    private DependencyList scanDependencies(Method implementationMethod, int dependencyLevel, List<Method> callStack) {
        ...
        for (int i = 0; i < instructionList.size(); i++) {
            ...
            if (ByteCodeUtils.isServiceIntfCall(sourceClazz, instruction, theConstPoolGen)) {
                if (instruction instanceof InvokeInstruction) {
                    String invokeTypeName = ((InvokeInstruction) instruction).getReferenceType(theConstPoolGen).toString();
                    String invokeMethodName = ((InvokeInstruction) instruction).getMethodName(theConstPoolGen);
                    if ("com.ykb.adc.internet.prepaid.service.intf.IIntPrepaidServiceProxy".equals(invokeTypeName)) {
                        System.out.println(invokeTypeName);
                    }
                }
    ...
    public DependencyScanResult scanDependencies(JarFile jarFile) {
        ...
        //
        IJarFileScanner jarScanner = new DefaultAppJarScanner(FileSystemClassLoader.getClassLoader());
